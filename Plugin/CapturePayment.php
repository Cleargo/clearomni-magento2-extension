...
class CapturePayment
{
    ...
    )
    {
        ...
        $query->bindValue(2,$subject->getId().' '.$subject->getStatus().' '.$result->getStatus());
        ...
        $query->execute();
        if($comment=='Payment is authorised waiting for capture'||$comment=='Adyen Payment Successfully completed'){
            if($comment=='Payment is authorised waiting for capture') {
